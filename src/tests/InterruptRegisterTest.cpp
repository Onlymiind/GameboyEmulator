...
TEST_CASE("interrupt register") {
    ...
    REQUIRE(reg.getFlags() ==
            (gb::g_unused_interrupt_bits |
             static_cast<uint8_t>(gb::InterruptFlags::LCD_STAT)));
    ...
    reg.write(0x0000, gb::g_unused_interrupt_bits |
                          static_cast<uint8_t>(gb::InterruptFlags::VBlank));
    REQUIRE(reg.read(0x0000) == reg.getFlags());
    REQUIRE(reg.getFlags() ==
            (gb::g_unused_interrupt_bits |
             static_cast<uint8_t>(gb::InterruptFlags::VBlank)));
